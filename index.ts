...
const socket = new WebSocket(
  `wss://misskey.io/streaming?i=${process.env.MISSKEY_API_KEY}`
);

const regex =
  /(([\u3041-\u3096]|[\u30A1-\u30FA]|[々〇〻\u2E80-\u2FDF\u3400-\u4DBF\u4E00-\u9FFF\uF900-\uFAFF]|[\uD840-\uD87F][\uDC00-\uDFFF]){3,})(?!\1).{2,}\1/g;
...
socket.onmessage = function (event) {
  ...
  if (
    !(eventData.body.body.visibility === "public") &&
  ...
  if (
    eventData.body.body.text === "@dance_robot_dance フォローミー" ||
  ...
  if (
    eventData.body.body.text.match(
      /[『』（）「」。、a-zA-Z0-9!-/:-@¥[-`{-~]/g
    ) !== null ||
    result === null
  ) {
